...
def regression_Layer(X, Y, X_te, Y_te, scores_file_1, scores_file_2, model_file):
    
    # if layer_name != 'VisionTransformer-1':
    #     mask = np.load(f'{maskdir}{layer_name}/Folder_5_{remain_rate}.npy')
    #     Y_trn = Y_trn[:, mask]
    #     Y_test = Y_test[:, mask]
    ...
    Y = preprocess_pipeline.fit_transform(Y)
    ...
    for epoch in (range(num_epochs)):
        ...
        if (epoch+1)%5==0:
            ...
            print(f'Prediction accuracy is: {np.mean(rs):3.3}\tr2_score is: {r2:3.3}')
    np.save(scores_file_1,scores1)
    np.save(scores_file_2,scores2)
    torch.save({
                'epoch': epoch,
...
def main():
    ...
    parser.add_argument(
        '--Layers',  
        nargs = '+',
        default=['Linear-2', 'Linear-4', 'Linear-6', 'Linear-8', 'Linear-10', 'Linear-12'], # ['Linear-2', 'Linear-4', 'Linear-6', 'Linear-8', 'Linear-10', 'Linear-12', 'VisionTransformer-1']
        type=str
    )
    ...
    maskdir = f'{project_path}/data/proj_feat/{subject}/{fetch_gt}/index_each_{fetch_fmri}/'
    ...
    for layer in Layers:
        Y = np.load(f'{featdir}/{layer}/tr_each.npy').astype("float32").reshape([X.shape[0],-1])
        Y_te = np.load(f'{featdir}/{layer}/te_ave.npy').astype("float32").reshape([X_te.shape[0],-1])
        print(f'Now making decoding model for... {subject}:  {fetch_fmri}, {fetch_gt}')
        print(f'X {X.shape}, Y {Y.shape}, X_te {X_te.shape}, Y_te {Y_te.shape}')
        os.makedirs(f'{savedir_model}/{layer}', exist_ok=True)
        os.makedirs(f'{savedir_scores}/{layer}', exist_ok=True)
        model_file = f'{savedir_model}/{layer}/{fetch_fmri}_{method}.pth'
        scores_file_1 = f'{savedir_scores}/{layer}/{fetch_fmri}_{method}_1.npy'
        scores_file_2 = f'{savedir_scores}/{layer}/{fetch_fmri}_{method}_2.npy'
        regression_Layer(X, Y, X_te, Y_te, scores_file_1, scores_file_2, model_file)
        del Y, Y_te
...
'''
python train_guidance.py --fetch_gt g --fetch_fmri nsdgeneral --subject <subject> --method neuraldiffuser
'''